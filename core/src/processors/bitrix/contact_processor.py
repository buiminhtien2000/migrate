# src/processors/contact_processor.py
...
class ContactProcessor(BaseProcessor):
    ...
    async def _transform_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        field_mapping = self.mapping_fields['contact'] or {}
        ...
        transformed['NAME'] = f"{data.get('LAST_NAME')} {data.get('NAME')}"   
             
        if data.get('COMPANY_IDS'):
            transformed['COMPANY_IDS'] =  await self.get_mapping_id('CompanyProcessor', data.get('COMPANY_IDS'))
